...
# 添加 root 参数，指定模型根目录为 F:\insightface_models
app = FaceAnalysis(
    allowed_modules=['detection', 'landmark_2d_106'],
    root='F:/insightface_models'  # 或使用双反斜杠 'F:\\insightface_models'
)
...
parser.add_argument('--loss_combination', default='all', 
                   ...
                   help='损失函数组合选择')
parser.add_argument('--data_dir', default='./datasets/train', help='训练数据目录')
...
def train(input_generator_size=160, batch_size=16, alpha=4e-4,
    ...
    train_iter = DataLoader(train_dataset, batch_size=32, shuffle=True, num_workers=4, pin_memory=True)
    ...
    while epoch.n < num_epochs_predis:
        for X in train_iter:
            ...
            fake_img = output_G
            # 阶段 2 中的调用代码
            loss_landmark = Discriminator_Network_Loss(real_img, fake_img, app)  # 传入 app
    ...
    while epoch.n < num_epochs_step3:
        for X in train_iter:
            ...
            optimizer_D.zero_grad()
            loss_d.backward(retain_graph=True)  # 添加 retain_graph=True
            optimizer_D.step()
...
if __name__ == '__main__':
    ...
    train(batch_size=32, num_epochs_pregen=args.phase1, num_epochs_predis=args.phase2, num_epochs_step3=args.phase3,
          ...
          loss_combination=args.loss_combination, data_dir=args.data_dir,
          model_version=args.model_version,
          test_dir=args.test_dir, result_dir=args.result_dir)