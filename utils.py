...
def Generator_Network_Loss(real_img, fake_img, mask, app):
    base_loss = F.mse_loss(fake_img * mask, real_img * mask)
    # 在这里导入 HybridLoss 以避免循环导入
    from losses import HybridLoss
    hybrid_loss = HybridLoss()(real_img, fake_img, app) * 0.5  # 添加权重系数
    return base_loss + hybrid_loss
...
def Discriminator_Network_Loss(real_img, fake_img, app):
    from losses import HybridLoss
    return HybridLoss()(real_img, fake_img, app)  # 使用默认权重
...
class MyDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        # 同时支持jpg和png格式的图片
        jpg_list = list(images_path.glob('*.jpg'))
        png_list = list(images_path.glob('*.png'))
        images_list = jpg_list + png_list
        
        if len(images_list) == 0:
            print(f"警告：在{root}目录下未找到任何.jpg或.png图片文件")
    ...
    def __getitem__(self, item):
        ...
        try:
            ...
            try:
                ...
                print(f"使用OpenCV读取图片 {image_path} 时也出错: {e2}")
                # 创建一个空白图片作为备用
