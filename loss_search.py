...
class LossSearchExperiment:
    ...
    def __init__(self, config):
        ...
        # 初始化insightface (用于landmark损失)
        print("初始化FaceAnalysis...")
        self.app = FaceAnalysis(
            allowed_modules=['detection', 'landmark_2d_106'],
            root='F:/insightface_models'
        )
    ...
    def select_best_combination(self):
        ...
        # 计算综合得分 (PSNR越高越好, SSIM越高越好, FID越低越好)
        scores = {}
        for combo, result in self.results.items():
            ...
            # 综合得分公式: 可根据需要调整权重
            # 标准化各指标并加权求和
            psnr_score = metrics.get('psnr', 0) / 30.0  # 假设PSNR最大约30
            ssim_score = metrics.get('ssim', 0)  # SSIM本身在0-1
            fid_score = 1.0 - min(metrics.get('fid', 100), 100) / 100.0  # FID越低越好
    ...
    def train_full_dataset(self, best_combination, num_epochs):
        ...
        # 复制最终模型到主目录
        # train_with_loss_combination 将模型保存在 {experiment_dir}/{best_combination}/final_model.pth
        trained_model_path = os.path.join(self.experiment_dir, best_combination, 'final_model.pth')
        if os.path.exists(trained_model_path):
            shutil.copy(trained_model_path, os.path.join(self.experiment_dir, 'best_final_model.pth'))
            print(f"最终模型已复制至: {os.path.join(self.experiment_dir, 'best_final_model.pth')}")
        else:
            print(f"警告: 未找到模型文件 {trained_model_path}，跳过复制")
        
        return G, final_metrics


def main():
    parser = argparse.ArgumentParser(description='损失函数组合自动搜索')
    ...
    parser.add_argument('--combinations', nargs='+', default=None, 
                       help='指定要测试的损失组合，默认测试所有')
    parser.add_argument('--calculate_fid', action='store_true', default=True, help='是否计算FID')
    ...
    config = {
        ...
        'mpv': [0.5109094257003173, 0.41451381534316145, 0.3723845290805799],  # CelebA-HQ均值
    }
